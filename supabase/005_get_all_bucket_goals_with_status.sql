-- ============================================
-- Function to get all active bucket goals with current status
-- ============================================

CREATE OR REPLACE FUNCTION get_all_bucket_goals_with_status(
  p_user_id UUID
)
RETURNS TABLE (
  id INT,
...
BEGIN
    ...
    COALESCE(
      (
        SELECT
          ...
          COALESCE(SUM(
            CASE
              WHEN t.to_bucket_id = bg.bucket_id THEN t.amount
              ELSE 0
            END
          ), 0)
          ...
          - COALESCE(SUM(
            CASE
              WHEN t.from_bucket_id = bg.bucket_id THEN t.amount
              ELSE 0
            END
          ), 0)
        FROM transaction t
        WHERE
          t.user_id = p_user_id
          AND (t.from_bucket_id = bg.bucket_id OR t.to_bucket_id = bg.bucket_id)
  ...
  FROM bucket_goal bg
  INNER JOIN bucket b ON bg.bucket_id = b.id
